...
import Config
from models.MHGNNs import OAMHRGAT
from models.MHGNNs import AMHRGAT
...
class RobertaAMAMP(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        self.gnn = AMHRGAT(gnn_layers_num, self.config.hidden_size, base_num=Config.model_args.base_num,
                        num_rels=Config.rgcn_relation_nums, k_hop=Config.k_hop, alpha=Config.alpha, negative_slope=Config.negative_slope,
                        theta=Config.theta)
    ...
    def forward(
        ...
        graphs_representations = self.gnn(graphs, question_origin_representations, base_nodes_ids, exten_nodes_ids, exten_edges_ids,
...
class RobertaOAMAMP(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.gnn = OAMHRGAT(gnn_layers_num, self.config.hidden_size, base_num=Config.model_args.base_num,
                        num_rels=Config.rgcn_relation_nums, k_hop=Config.k_hop, alpha=Config.alpha, negative_slope=Config.negative_slope,
                        theta=Config.theta)
